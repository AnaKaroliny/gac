...
public class ProcessoDAO implements Serializable {
	...
	public List<Processo> findByMatricula(int first, int pageSize, ProcessoFilter filter) {
		return manager.createQuery("select p from Processo p where p.usuario.matricula = :matricula", Processo.class)
	...
	public Long encontrarQuantidadeProcesso(ProcessoFilter filter) {
		return manager.createQuery("select count(p) from Processo p where p.usuario.matricula = :matricula", Long.class)
	...
	public Processo findProcessoById(String id) {
		try {
			return manager.createNamedQuery("Processo.findById", Processo.class)
